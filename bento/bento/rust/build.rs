...
fn main() {
    ...
    for arg in shlex::split(std::str::from_utf8(&output.stdout).unwrap()).unwrap() {
        if !(arg.to_string()==String::from("-mpreferred-stack-boundary=3")||arg.to_string()==String::from("-mindirect-branch=thunk-extern")||arg.to_string()==String::from("-mindirect-branch-register")||arg.to_string()==String::from("-fno-allow-store-data-races")||arg.to_string()==String::from("-fconserve-stack")||arg.to_string()==String::from("-mrecord-mcount"))
        {
        builder = builder.clang_arg(arg.to_string());
        }
    }
    builder = builder.clang_arg(String::from("-I/usr/lib/modules/5.15.0/source/include/"));
